OC.L10N.register(
    "files_paperhive",
    {
    "Show Paper" : "Papier anzeigen",
    "Visit PaperHive at" : "PaperHive besuchen auf",
    "and transform reading into a process of collaboration!" : "und ändern Sie das Lesen in einen Prozess der Zusammenarbeit!",
    "Your DocID" : "Ihre DocID",
    "is the last fragment of PaperHive document URL." : "ist der größere Teil der PaperHive-Dokumenten-URL.",
    "Exemplary URL" : "Exemplarische URL",
    "An error occurred!" : "Ein Fehler ist aufgetreten!",
    "Discuss" : "Diskutieren",
    "Would you like to open in new window?" : "Möchten Sie ein neues Fenster öffnen?",
    "PaperHive" : "PaperHive",
    "DocID" : "DocID",
    "Saving..." : "Speichervorgang…",
    "Please reload the page, error occured" : "Bitte Seite neuladen, ein Fehler ist aufgetreten",
    "Cannot add your PaperHive document with BookID {id}. {message}" : "Ihr PaperHive-Dokument mit der BookID {id} konnte nicht hinzugefügt werden. {message}",
    "No such document found in database." : "Dokument nicht in Datenbank gefunden.",
    "Problem connecting to PaperHive." : "Problem beim Verbinden zu PaperHive",
    "Received wrong response from PaperHive." : "Falsche Rückmeldung von PaperHive erhalten.",
    "Problem connecting to PaperHive to fetch discussions." : "Problem beim Verbinden zu PaperHive, um Diskussion zu laden.",
    "The file already exists." : "Die Datei existiert bereits.",
    "The file is locked." : "Die Datei ist gesperrt.",
    "An internal server error occurred." : "Ein interner Serverfehler ist aufgetreten."
},
"nplurals=2; plural=(n != 1);");
